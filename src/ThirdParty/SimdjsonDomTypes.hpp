...
void getValue(value_type& data, simdjson::dom::element jsonData) {
	uint64_t newValue{};
	if (auto result = jsonData.get(newValue); result) {
		throwError(result);
	}
...
template<> void getValue(std::nullptr_t&, simdjson::dom::element value) {
	if (auto result = value.is_null(); !result) {
		throwError(result);
	}
}
...
template<map_t map_type> void getValue(map_type& map, simdjson::dom::element json_value) {
	...
	for (auto field : object) {
		...
		auto key_result = field.key;

		key = static_cast<typename map_type::key_type>(std::string(key_result));
...
template <size_t currentIndex = 0> void getValue(tuple_type& out_value, simdjson::dom::element json_value) {
...
namespace SimdjsonDomTypes {
	...
	public:
		...
		void streamDoc(dom::element element, std::ostream& s) const
		{
			switch (element.type())
			{
		...
		virtual bool Parse(const char* json, size_t length, std::unique_ptr<ParseResultBase>& reply) const override
		{
			...
			auto error = parser.parse(result->json).get(result->doc);
			value_type value;
			if (!error) {
				simdjson::dom::element valueDoc{ result->doc };
		...
		virtual bool Prettify(const ParseResultBase& parseResult, std::unique_ptr<StringResultBase>& reply) const override
		{
			...
			dom::document          doc;
			auto error = parser.parse(input.json).get(doc);
			if (!error) {
				dom::element const val = doc;
